...
const ProjectContainer = () => {
  ...
  const searchForTheMembersProjects = async () => {
    var joinedProjectData = {};
    setUsersId([]);
    usersId.map((userId) => {
      var projectRef = firestore
        .collection("users")
        .doc(userId)
        .collection("projects");
            ...
            .then((snapshots) => {
              snapshots.forEach((docc) => {
                ...
                docc.ref.parent.parent.get().then((dc) => {
                  joinedProjectData = dc.data();
                  setProjectData((prevData) => [
                    ...prevData,
                    joinedProjectData,
                  ]);
                });
                // console.log(typeof joinedProjectData, "=>", joinedProjectData);
              });
            });
        });
      });
    });
    // setProjectData((prevData) => [...prevData, joinedProjectData]);
  };
  ...
  useEffect(() => {
    ...
    getUsersID();
    getMemberEmail();
    getTheCheckedProject().then(searchForTheMembersProjects());
    
    //Grab project info from firestore
    firestore
      .collection("users")
      .doc(id)
      .collection("projects")
      ...
      .then((snapshots) => {
        if (snapshots.size > 0) {
          snapshots.forEach((project) => {
            ...
            projectRef.doc(project.id).set(finalProjectData, { merge: true });
            console.log("DONE");
          });
        } else {
          console.log("failed");
      ...
      .then((snapshots) => {
        if (snapshots.size > 0) {
          snapshots.forEach((project) => {
            ...
            console.log("all projects doc are not checked Successfully");
          });
        }
      });
  };
            ...
            <div key={index}>
              <div className="project-container-listContainer">
                <div className="project-list-container">
                  ...
                  <div className="project-container-chooseButton" key={index}>
                    {chosen[index] ||
